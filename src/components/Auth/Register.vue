...
<template>
    <div id="container" class="flex min-h-screen">
        <aside class="flex flex-col px-14 p-8 w-[60%] text-[var(--text-primary)]">
            ...
            <p class="leading-[50px]">Đăng ký để tiếp tục</p>
            ...
            <form class="flex flex-col [&>div.input-wrapper:not(:last-child)]:mb-6 mt-4">
                ...
                <div class="relative flex input-wrapper password-input-container">
                    ...
                    <input id="password-input" type="password" class="flex-1 border-[var(--text-primary)] bg-[#1330200D] px-16 border rounded-lg h-[60px] placeholder:text-[var(--text-primary)] outline-none" placeholder="Mật khẩu" @keyup="haveEnterPass()" @keyup.enter="registerHandle()" v-model="password">
                ...
                <!-- Error? Are u forgot ur pass? -->
                 <p id="error-element" class="hidden mb-4 text-left text-red-600">Error</p>

                <!-- Link start! -->
                <button class="rounded-lg h-[60px] font-bold bg-[var(--text-primary)] text-lg text-white" type="button" @click="registerHandle()">Đăng ký</button>
            </form>

            <!-- Register Redirect -->
            <RouterLink to="/login" class="mt-auto text-center">Bạn đã có tài khoản?</RouterLink>
